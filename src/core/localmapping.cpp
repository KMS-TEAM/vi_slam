...
namespace vi_slam{
    namespace core{
        LocalMapping::LocalMapping(System* pSys, Atlas *pAtlas, const float bMonocular, bool bInertial, const string &_strSeqName, optimization::GTSAMOptimizer *gtsam_optimizer):
                mpSystem(pSys), mbMonocular(bMonocular), mbInertial(bInertial), mbResetRequested(false), mbResetRequestedActiveMap(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas), bInitializing(false),
                mbAbortBA(false), mbStopped(false), mbStopRequested(false), mbNotStop(false), mbAcceptKeyFrames(true),
                mbNewInit(false), mIdxInit(0), mScale(1.0), mInitSect(0), mbNotBA1(true), mbNotBA2(true), infoInertial(Eigen::MatrixXd::Zero(9,9)), gtsam_optimizer_(gtsam_optimizer)
        {
            mnMatchesInliers = 0;
                ...
                if(CheckNewKeyFrames() && !mbBadImu)
                {
                    ...
                    if(!CheckNewKeyFrames() && !stopRequested())
                    {
                        if(mpAtlas->KeyFramesInMap()>2)
                        {
                            ...
                            {
                                optimization::Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame,&mbAbortBA, mpCurrentKeyFrame->GetMap(),num_FixedKF_BA,num_OptKF_BA,num_MPs_BA,num_edges_BA, gtsam_optimizer_);
                                b_doneLBA = true;
                            }
        ...
        void LocalMapping::CreateNewMapPoints()
        {
            ...
            geometry::FMatcher matcher(th,false);
            ...
            for(size_t i=0; i<vpNeighKFs.size(); i++)
            {
                ...
                geometry::Camera* pCamera1 = mpCurrentKeyFrame->mpCamera;
                geometry::Camera* pCamera2 = pKF2->mpCamera;
        ...
        void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
        {
            ...
            if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
            {
                ...
                mRwg = basics::converter::toMatrix3d(cvRwg);
                mTinit = mpCurrentKeyFrame->mTimeStamp-mFirstTs;
            }
            else
            {
                mRwg = Eigen::Matrix3d::Identity();
                mbg = basics::converter::toVector3d(mpCurrentKeyFrame->GetGyroBias());
                mba = basics::converter::toVector3d(mpCurrentKeyFrame->GetAccBias());
            }
            ...
            {
                mpAtlas->GetCurrentMap()->ApplyScaledRotation(basics::converter::toCvMat(mRwg).t(),mScale,true);
            ...
            if (!mpAtlas->isImuInitialized())
            {
                ...
                mpTracker->t0IMU = mpTracker->mCurrentFrame.time_stamp_;
