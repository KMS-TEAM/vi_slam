...
namespace vi_slam
{
    namespace datastructures {
        ...
        KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) :
                bImu(pMap->isImuInitialized()), mnFrameId(F.id_),  mTimeStamp(F.time_stamp_), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
                ...
                mpORBvocabulary(F.mpVocaburary), mbFirstConnection(true), mpParent(NULL), mDistCoef(F.mDistCoef), mbNotErase(false), mnDataset(F.mnDataset),
                ...
                mvKeysRight(F.keypointsRight_), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.mTrl), mnNumberOfOpt(0) {
        ...
        void KeyFrame::SetORBVocabulary(DBoW3::Vocabulary* pORBVoc)
        {
